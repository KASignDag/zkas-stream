...
// Remove after production storage reports 679 rows.
const newestSep6Trade = {
  timestamp: Date.parse('2026-09-06T12:01:00Z'),
  side: 'sell',
  zkasAmount: 3500,
  priceKas: 177.333345 / 3500,
  totalKas: 177.333345,
};
...
async function handleGet({ request, env, waitUntil }) {
  ...
  if (!endpoint) {
    if (env.OTC_TRADES) {
      ...
      if (Array.isArray(stored?.trades) && stored.trades.length === 678) {
        stored = { schemaVersion: 1, updatedAt, trades: [...stored.trades, newestSep6Trade].sort((a, b) => a.timestamp - b.timestamp) };
